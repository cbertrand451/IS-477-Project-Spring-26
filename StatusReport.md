...
- **Colin**: Finish cleaning the rookie stats dataset. Once both datasets are ready, integrate data based on season. There may be multiple players per season, meaning that each individual player will have the correlating season outcome. 
...
**Colin**: I manually copied the nba draft data for the Chicago Bulls and saved it in a file called **draft_data.csv** in the **data** folder. I then sorted it for players that actually played for the Chicago Bulls (traded players were marked by a special character). I got rid of drafts that the Bulls didn't select players, as those were imported as null rows. I used a jupyter notebook to clean and prepare the dataset. I then made a folder within the **data** folder called **rookie_stats_data**, to house the rookie stats for each player. I used the same notebook to programatically filter through all of the players and make a folder in the rookie data directory. I then manually copy and pasted all of the players' rookie year stats, since basketball-reference.com no longer was allowing for web scraping. I then cleaned each player's stats dataset by looping through each dataset and looking for missing columns and completely empty datasets. Those datasets were filled with the missing columns and given null values. I then got rid of the old stats from the draft dataset, and imported all of the rookie year stats using the player name as the key value. I saved the updated draft dataset in the **data** folder as **draft_data_cleaned.csv**.
